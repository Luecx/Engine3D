...
template<int M, int N> class Matrix {
    ...
    void clear() { std::memset(values, 0, M * N * sizeof(Precision)); }
    ...
    Matrix<M,N> invert(){
        ...
        if constexpr (M == 4){
            ...
            Precision det = (*this)[0] * inv[0] + (*this)[1] * inv[4] + (*this)[2] * inv[8] + (*this)[3] * inv[12];
    ...
    Precision&           operator()(int m, int n) { return values[m][n]; }
    Precision            operator()(int m, int n) const { return values[m][n]; }
    Precision&           operator[](int index) { return values[index % M][index / N]; }
    Precision            operator()(int index) const { return values[index % M][index / N]; }
